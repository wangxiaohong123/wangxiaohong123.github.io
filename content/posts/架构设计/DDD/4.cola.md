...
*   com.xxx.xxx
    *   adapter
        *   controller    ←只负责“入站”（别人调用我）
        *   dubbo    ←Dubbo Provider 接口impl
        *   consumer    ← MQ 消费者
        *   scheduler    ← 定时任务
    ...
    *   domain
        ...
        *   port    ← 其他领域接口声明（Dubbo Consumer、mq接口、数据库操作包装）
            *   dubbo     ← Dubbo Consumer接口声明
            *   producer    ← mq producer接口声明
            *   repository    ← 数据库操作接口声明
        
    *   infra
        
        *   assembler    ← do和领域对象转换
        
        *   repository    ← 数据库操作接口实现
        *   dubbo    ← Dubbo Consumer接口实现
        *   producer    ← mq producer接口实现
        *   config
