...
ConutDownLatch和CyclicBarrier是实现线程同步的两个工具（虽然Future也可以）， CountDownLatch 主要用来解决一个线程等待多个线程的场景；而 CyclicBarrier 是一组线程之间互相等待。除此之外 CountDownLatch 的计数器是不能循环利用的，也就是说一旦计数器减到 0，再有线程调用 await()，该线程会直接通过。但 CyclicBarrier 的计数器是可以循环利用的，而且具备自动重置的功能，一旦计数器减到 0 会自动重置到你设置的初始值。除此之外，CyclicBarrier 还可以设置回调函数 。
...
// 队列2
Vector queue2;
// 执行回调的线程池，里面只有一个线程，保证并发时数据不一致
Executor executor = Executors.newSingleThreadExecutor();
// 初始化CyclicBarrier传了两个参数：‘2’和回调函数；2表示等待两个线程，当有两个线程执行barrier.await()方法后，开始执行回调函数
